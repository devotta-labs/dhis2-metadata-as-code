...
That invokes `declare-cli check` locally. It loads `declare.config.ts`, regenerates `declare-env.d.ts` for the declared `target`, and validates the schema with Zod. Treat a clean `pnpm check` as the gate for "done".